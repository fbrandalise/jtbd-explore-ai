...
-- View hierárquica (JSON) para carregar o Job Map de uma vez
create or replace view vw_jobs_hierarchy as
select jsonb_agg(
  jsonb_build_object(
    'id', bj.slug, 'name', bj.name, 'description', bj.description, 'status', bj.status, 'order', bj.order_index,
    'littleJobs', (
      select jsonb_agg(
        jsonb_build_object(
          'id', lj.slug, 'name', lj.name, 'description', lj.description, 'status', lj.status, 'order', lj.order_index,
          'outcomes', (
            select jsonb_agg(
              jsonb_build_object(
                'id', oc.slug, 'name', oc.name, 'description', oc.description, 'status', oc.status, 'order', oc.order_index, 'tags', oc.tags
  ...
  ) order by bj.order_index
) as hierarchy
from big_jobs bj
left join little_jobs lj on lj.big_job_id = bj.id
left join outcomes oc on oc.little_job_id = lj.id
group by 1;